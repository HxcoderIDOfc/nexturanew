...
const child = spawn(process.execPath, ["koyeb-base.js"], {
  env: { ...process.env, PORT: String(CORE_PORT), HOST: "127.0.0.1" },
  stdio: "inherit"
});
child.on("exit", (code, signal) => {
  console.error(`[koyeb-max] core berhenti. code=${code} signal=${signal}`);
  process.exit(code ?? 1);
});
...
async function buildTaskBrief(input, headers, profile) {
  const prompt = maxBriefPrompt(latestUserText(input.messages || []), profile);
  const briefBody = {
...
async function verifyAnswer(input, headers, answer, policy, passes) {
  ...
  for (let pass = 0; pass < passes; pass++) {
    const body = {
      ...
      thinking_level: pass === 0 ? "cepat" : "sedang",
      search: false,
      agent_search: false,
      review: false,
      stream: false,
...
async function handleChat(req, res) {
  try {
    ...
    messages.unshift({ role: "system", content: policy.instruction });
    ...
    data[metaKey] = {
      ...
      max_auto_search: !explicitSearch && search
    };
...
server.listen(PUBLIC_PORT, HOST, () => {
  console.log(`[koyeb-max] Nextura Max Engine online di http://${HOST}:${PUBLIC_PORT}`);
  console.log(`[koyeb-max] Base router di http://127.0.0.1:${CORE_PORT}`);
  console.log(`[koyeb-max] Adaptive quality enabled=${MAX_ENGINE_ENABLED}`);
});
