...
function healthPayload(index = null) {
  ...
  return { ok: true, service: "Nextura WhatsApp Bot", health: index, whatsapp: wa.status, connected: wa.status === "connected", auto_read: wa.autoRead, plugins: wa.pluginCount || 0, uptime_seconds: Math.floor((Date.now() - startedAt) / 1000), timestamp: new Date().toISOString() };
}
...
server.listen(PORT, HOST, () => console.log(`[nextura-wa] online http://${HOST}:${PORT} · /wa · /plugins · /console`));
